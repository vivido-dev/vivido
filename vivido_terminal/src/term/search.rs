...
mod tests {
    ...
    fn regex_right() {
        ...
        let mut regex = RegexSearch::new("Viv.*123").unwrap();
    ...
    fn nested_regex() {
        ...
        let mut regex = RegexSearch::new("Viv.*ido").unwrap();
        ...
        let mut regex = RegexSearch::new("Viv[^o]*ido").unwrap();
        let start = Point::new(Line(0), Column(0));
        let end = Point::new(Line(0), Column(12));
        assert_eq!(term.regex_search_right(&mut regex, start, end), Some(start..=end));
    }
    ...
    #[test]
    fn skip_dead_cell() {
        let term = mock_term("vivido");

        // Make sure dead state cell is skipped when reversing.
        let mut regex = RegexSearch::new("vivido").unwrap();
        let start = Point::new(Line(0), Column(0));

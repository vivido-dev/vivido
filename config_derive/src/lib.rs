//! Vivido's in-package configuration derives.
//!
//! Adapted from `alacritty_config_derive` 0.2.5, licensed under Apache-2.0 OR MIT.
    ...
    for generic in params {
        ...
        if let GenericParam::Type(TypeParam { ident, .. }) = generic {
            ...
            generics.constrained.extend(quote! {
                #ident : Default + serde::Deserialize<'de> + vivido_config::SerdeReplace,
            });

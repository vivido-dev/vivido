...
const WESTON_GLOBAL_PREFIX: &str = "weston_";

fn has_weston_global<'a>(interfaces: impl IntoIterator<Item = &'a str>) -> bool {
    interfaces.into_iter().any(|interface| interface.starts_with(WESTON_GLOBAL_PREFIX))
}

fn is_weston_compositor(globals: &GlobalList) -> bool {
    globals.contents().with_list(|globals| {
        has_weston_global(globals.iter().map(|global| global.interface.as_str()))
    })
}
...
pub struct WinitState {
    ...
    /// Whether CSD move requests must use the original button-press dispatch.
    pub immediate_csd_move: bool,
...
impl WinitState {
    pub fn new(
        ...
        Ok(Self {
            ...
            immediate_csd_move: is_weston_compositor(globals),
...
mod tests {
    ...
    #[test]
    fn identifies_weston_private_globals() {
        assert!(has_weston_global(["wl_compositor", "weston_capture_v1"]));
        assert!(has_weston_global(["weston_desktop_shell"]));
    }

    #[test]
    fn leaves_other_compositors_on_deferred_csd_moves() {
        assert!(!has_weston_global(["wl_compositor", "xdg_wm_base"]));
    }
}

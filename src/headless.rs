...
pub fn run(mut options: Options) -> Result<(), Box<dyn Error>> {
    ...
    paths.prepare_endpoint(&session)?;
...
struct Readiness {
    pipe: std::cell::RefCell<Option<OwnedFd>>,
}

impl Readiness {
    fn new(pipe: OwnedFd) -> Self {
...
fn report_failure(pipe: &OwnedFd, diagnostic: &str) -> io::Result<()> {
...
fn write_all(pipe: &OwnedFd, bytes: &[u8]) -> io::Result<()> {
...
fn readiness_pipe() -> io::Result<(OwnedFd, OwnedFd)> {

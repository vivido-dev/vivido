...
use crate::terminal::tty::Options;
use crate::terminal::tty::windows::blocking::{UnblockedReader, UnblockedWriter};
use crate::terminal::tty::windows::child::ChildExitWatcher;
use crate::terminal::tty::windows::{Pty, cmdline, win32_string};
...
impl Drop for Conpty {
    fn drop(&mut self) {
        // This blocks until the conout pipe is drained, and would deadlock if that pipe had
        // already been dropped. `ConptyBackend` owns both and orders the fields so this close
        // always runs first.
        //
...
pub fn new(config: &Options, window_size: WindowSize) -> Result<Pty> {
    ...
    // PTY process does not inherit any handles from this Vivido process.

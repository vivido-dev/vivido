...
use crate::terminal::index::{self, Boundary, Column, Direction, Line, Point};
...
impl<T> Term<T> {
    ...
    ///
    /// User-controlled overlays like `Selection` are **not** part of the
    ...
    /// data into the protocol-neutral command model.
    pub fn submit_graphics(&self, command: GraphicsCommand)
    where
        T: EventListener,
    {
        self.event_proxy.send_event(Event::Graphics(command));
    }
...
///
/// This returns the current terminal version as a unique number based on vivido_terminal's
...
mod tests {
    ...
    fn damage_cursor_movements() {
        ...
        // damage information (like the cursor), which we're not testing.
    ...
    fn parse_cargo_version() {
        assert_eq!(version_number(env!("CARGO_PKG_VERSION")), 1_00);
